...
class Portfolio():
    ...
    def _calibrate_dates(self):
        common_dates = set.intersection(
            *[set(asset.index.unique()) for asset in self.assets])
        self.assets = [asset[asset.index.map(
            lambda x: x in common_dates)] for asset in self.assets]

    def _calculate_asset_development(self, idx, weighted=True):
        values = []
        if weighted:
            stocks = self.start_investment * \
                self.weights[idx] / self.assets[idx]["Close"].iloc[0]
        else:
    ...
    def plot_comparison(self):
        ...
        self.hist.plot(
            label="Portfolio",
            title="Simulation of Portfolio vs. 100%% Initial Investment on individual stocks\n Simulated Investment = %.2f" %
            self.start_investment)
        plt.grid()
        plt.legend()
    ...
    def summary(self):
        ...
        t2 = "Capital Development: from %.2f to %.2f" % (
            self.hist.iloc[0], self.hist.iloc[-1]) + "\n"
        t3 = "Yield: %.4f" % yy + "\n"
        t4 = "Gain: %.4f; From which Dividends: %.4f; and Asset Price Development: %.4f" % (
            win, dividends, win - dividends)
    ...
    def generate_saving_plan(self, monthly_investment):
        ...
        self.spar_dev = pd.Series(
            [monthly_investment * i for i in range(1, len(mindex - 1) + 1)], index=mindex)
